...
def DrawGraphics():
    ...
    for num in range(number_of_samples):
        ...
        figures.savefig('C:/Users/marin/OneDrive/Рабочий стол/куку/MatStat-main/Lab1-4/Res' +name_list[num] + str(sizes[size]) + ".png")
    return
    ...
    for num in range(number_of_samples):
        for size in range(len(sizes)):
            ...
            for kf in range(len(koeff)):
                y = get_pdf(x)
                sample = rvs_list[num]
                axs[kf].plot(x, y[num], color='pink', label='pdf')
                sns.kdeplot(data=sample, bw_method='scott', bw_adjust=koeff[kf], ax=axs[kf],
                            fill=False, common_norm=False, color='mediumpurple', linewidth=1.5, label='kde')
            ...
            plt.show()
            figures.savefig('C:/Users/marin/OneDrive/Рабочий стол/куку/MatStat-main/Lab1-4/Res' + name_list[num] + ' KDE ' + str(sizes[size]) + ".png")
    return
